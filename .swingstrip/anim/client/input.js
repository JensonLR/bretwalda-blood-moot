...
import { liveEnemies, sameSide } from "./targeting.js";
import { clearTapped, ensureKeyTracking, isActionDown, isActionHit } from "./bindings.js";
...
import { cutAt } from "./render/chain.js";
...
export function subscribeLock(onChange) {
    lockListeners.add(onChange);
    return () => { lockListeners.delete(onChange); };
}
export function getLockSnapshot() { return lockSnap; }
export function getServerLockSnapshot() { return "|0|0"; }
...
export function lockEngaged() { return lock.engaged; }
/**
...
let reticleEl = null;
let footEl = null;
export function setLockReticle(el) { reticleEl = el; }
export function lockReticle() { return reticleEl; }
export function setLockFootMark(el) { footEl = el; }
export function lockFootMark() { return footEl; }
...
export function requestTargetSwitch(dir) {
    lock.pending = dir;
    lock.flicks++;
    publishLock();
}
...
function lockScore(yaw, local, e) {
 ...
 */
function applySwitch(players, local, localId) {
    ...
    for (const id of liveEnemies(players, localId, local)) {
        ...
        if (distTo(local, players[id]) > LOCK_ACQUIRE_RANGE)
            continue;
        ...
        if (onSide) {
            ...
            if (!take || Math.abs(rel) < Math.abs(takeRel)) {
                take = id;
                takeRel = rel;
            }
        }
 ...
 */
function pickTarget(yaw, players, local, localId) {
    ...
    if (owed && owed !== lock.id) {
        lock.id = owed;
        lock.reason = "you parried him";
        return;
    }
    let best = null;
    ...
    for (const id of liveEnemies(players, localId, local)) {
        ...
        if (d > (id === lock.id ? LOCK_DROP_RANGE : LOCK_ACQUIRE_RANGE))
            continue;
        inRange++;
        if (id !== lock.id && Math.abs(shortestAngle(yaw, bearingTo(local, e))) > acquireCone(d))
            continue;
    ...
    if (!lock.id) {
        ...
        if (best)
            ...
            lock.reason = inRange ? `${inRange} in range, none inside the acquire cone` : "no live enemy in range";
        return;
    }
    if (!best || best === lock.id)
        return;
    if (bestScore < lockScore(yaw, local, players[lock.id]) * LOCK_HYSTERESIS)
        lock.id = best;
}
/**
 ...
 */
function updateLock(rig, players, local, localId, dt, enabled) {
...
export function beginSwingGesture(id, x, y) {
    swing.pressed = true;
    swing.dir = null;
    swing.owner = id;
    swing.x = x;
 ...
 */
export function trackSwingGesture(id, x, y) {
...
export function endSwingGesture() {
    swing.pressed = false;
    swing.owner = null;
}
/** The direction the last flick described, or null if the player only tapped. */
export function swingDirection() {
    return swing.dir;
}
const FEEL_KEY = "bretwalda.feel";
const DEFAULT_FEEL = { sensitivity: 1, shake: true, teamContrast: false };
let feel = DEFAULT_FEEL;
let feelLoaded = false;
const feelListeners = new Set();
function ensureFeelLoaded() {
    if (feelLoaded)
        return;
    feelLoaded = true;
    try {
...
export function subscribeFeel(onChange) {
    feelListeners.add(onChange);
    if (!feelLoaded) {
        ensureFeelLoaded();
        if (feel !== DEFAULT_FEEL)
            for (const l of feelListeners)
                l();
    }
...
export function getFeel() { ensureFeelLoaded(); return feel; }
export function getServerFeel() { return DEFAULT_FEEL; }
export function setFeel(next) {
    ...
    feelLoaded = true;
    try {
        window.localStorage.setItem(FEEL_KEY, JSON.stringify(feel));
    }
    catch { /* per-match only */ }
    for (const l of feelListeners)
        l();
}
// ---------------------------------------------------------------------------
...
export function subscribeHandedness(onChange) {
...
export function getServerHandedness() {
    return false;
}
export function setHandedness(next) {
    if (next === lefty)
        return;
    lefty = next;
    handLoaded = true;
 ...
 */
function applyFacingAssist(rig, players, local, localId, dt, isMobile) {
    ...
    for (const id of Object.keys(players)) {
        ...
        if (Math.abs(shortestAngle(rig.yaw, angle)) < ASSIST_CONE && (!best || d < best.d))
            best = { angle, d };
    }
    if (!best)
        return;
 ...
 */
let openCut = "right";
export function sampleInput(sources, rig, players, localId, dt, lastDir) {
    ...
    const down = (a) => isActionDown(a, mouse);
    /** Held now, or tapped since the last sample. */
    const hit = (a) => isActionHit(a, mouse);
    ...
    if ((pressedAttack || heavy) && alive)
        applyFacingAssist(rig, players, local, localId, dt, isMobile);
    ...
    if (chain === 0)
        openCut = asked ?? lastDir;
    const attackDir = asked && isMobile ? asked
    ...
    const sample = {
        ...
        message: {
            ...
            take: isMobile ? mobile.take : hit("take"),
            attackDir,
        },
    };
 ...
 */
export function useTouchControls(onCameraDrag) {
    ...
    const onTouchStart = useCallback((e) => {
        ...
        for (let i = 0; i < e.changedTouches.length; i++) {
            ...
            if (onMoveSide) {
                ...
                if (moveTouch.current !== null || touch.clientY < h * TOP_STRIP)
                    continue;
