...
import { WARRIOR_STATS, SWING_PHASES, SHOVE, KNOCKDOWN, EMOTE_SECONDS, SHIELD } from "../../types.js";
import { buildCharacter, buildWeaponForClass, buildOffhandFor, buildShield, shieldBoard, peopleOf, defaultAppearance, ELBOW_ALONG, KNEE_ALONG, GRIP_ALONG, GRIP_PITCH, reachOf } from "../characters.js";
import { getHandedness, subscribeHandedness } from "../input.js";
import { SWINGS, chainSwing, heavySwing } from "./chain.js";
/**
...
export function createMotion(p) {
    return {
        ...
        drapeX: new Array(DRAPE_BONES).fill(0),
        drapeXv: new Array(DRAPE_BONES).fill(0),
        drapeZ: new Array(DRAPE_BONES).fill(0),
        drapeZv: new Array(DRAPE_BONES).fill(0),
        draped: false,
    };
}
/**
...
export function createWarriorRig(parent, player, materials, settings) {
    const cls = player.warriorClass;
    const ap = player.appearance ?? defaultAppearance(cls);
    ...
    const noProxy = typeof window !== "undefined" && /[?&]shadowproxy=off(?:&|$)/.test(window.location.search);
    body.traverse((o) => {
    ...
    if (settings.shadows && !noProxy) {
        ...
        body.traverse((o) => {
            if (!(o instanceof THREE.Mesh) || !o.parent)
                return;
            const list = byBone.get(o.parent);
            if (list)
                list.push(o);
            else
                byBone.set(o.parent, [o]);
        });
 ...
 */
function weightLimb(geo, joint, band, upper, lower, hand) {
    if (geo.hasAttribute("skinIndex"))
        return;
 ...
 */
function weightDrape(geo, ring, half, base) {
    if (geo.hasAttribute("skinIndex"))
        return;
    const pos = geo.getAttribute("position");
    const n = pos.count;
    ...
    const at = (c, r) => base + 1 + c * DRAPE_RINGS + r;
    for (let i = 0; i < n; i++) {
 ...
 */
function hangCloak(pivot, base, bound, twin) {
    let lowest = 0;
    let half = 0;
    let deep = 0;
    ...
    for (const child of pivot.children.slice()) {
        ...
        const skinned = new THREE.SkinnedMesh(child.geometry, twin(child.material));
...
function articulate(built, twin) {
    ...
    for (const b of bound)
        b.mesh.bind(skeleton, b.at.matrixWorld.clone());
    return {
...
const approach = (cur, to, dt, rate) => cur + (to - cur) * Math.min(1, dt * rate);
/**
 ...
 */
const FACE_IDS = new Map();
const FACE_IDS_MAX = 256;
function faceIdentity(id) {
    const known = FACE_IDS.get(id);
    if (known !== undefined)
        return known;
    if (FACE_IDS.size >= FACE_IDS_MAX) {
        // Map iterates in insertion order, so the first key is the oldest.
        for (const stale of FACE_IDS.keys()) {
            FACE_IDS.delete(stale);
            break;
        }
    }
 ...
 */
export function settleDeath(motion, t) {
 ...
 */
function ingestNet(m, p, dtFrame, epoch) {
    ...
    if (epoch !== undefined)
        m.netEpoch = epoch;
    if (newest && newest.x === x && newest.z === z && newest.yawRaw === rawYaw && !confirmed)
        return false;
    ...
    if (newest) {
        ...
        if (teleported || gap <= 0 || gap > m.netInterval * 8) {
            ...
            if (spd > 0.5) {
                ...
                slots = clamp(clamp(Math.round(carried / m.netInterval), 1, 8), Math.max(1, byTiming - 1), byTiming + 1);
            }
    ...
    if (Math.abs(head.yaw) > 4096) {
        ...
        for (let k = 0; k < m.netCount; k++)
            snapAt(m, k).yaw -= turns;
    }
    return teleported;
}
/**
 ...
 */
export function cutNetHistory(m) {
    m.netCount = 0;
    m.netHead = 0;
    m.netArrive = 0;
    m.netJit = 0;
 ...
 */
function smoothNetError(m, dt, teleported) {
 ...
 * poseWarrior.
 */
export function stepWarriorTransform(rig, motion, player, dt, ctx, attacker) {
 ...
 */
function takeBearing(motion, attacker) {
 ...
 */
const whipOut = (x) => easeOutBack(Math.pow(x, 1.9));
/**
 ...
 */
function readSwing(motion, player, dt) {
    ...
    if (player.attackTimer > motion.swingPrev + 1e-3)
        motion.swingDur = nominal;
    if (player.attackTimer > motion.swingDur)
 ...
 */
function stanceLayer(st, ready, act, w) {
 ...
 */
function idleLayer(t, seed, wounded, w) {
 ...
 */
function gaitLayer(motion, speed, legLen, dt, w, armW = 1) {
    ...
    if (Math.floor((motion.stride - beat) / Math.PI) !== Math.floor((before - beat) / Math.PI))
        motion.land = 1;
...
function attackLayer(dir, ph, heavy, shielded, w, combo = 1) {
...
function blockLayer(hasShield, settle, w) {
 ...
 */
function shoveLayer(ph, shielded, w) {
 ...
 */
export function triggerEmote(motion, emote) {
    motion.emote = emote;
    motion.emoteT = 0;
}
/**
...
function staggerLayer(t, phase, motion, w) {
 ...
 */
function knockLayer(elapsed, downLeft, riseLen, fall, side) {
...
function dodgeLayer(phase, side, w) {
 ...
 * the body stands on.
 */
function settleOnFeet(legLen, plant, slack = 0) {
    ...
    if (reachL < reachR)
        P.llb = mix(P.llb, kneeFor(ax, P.llb, want / (latL || 1)), k);
    else
        P.lrb = mix(P.lrb, kneeFor(bx, P.lrb, want / (latR || 1)), k);
}
/**
...
function drapeCloak(rig, motion, dt, t, billow) {
    ...
    for (let c = 0; c < cols; c++) {
        ...
        const tz = clamp((Math.atan2(lx + rx, down) + sway) * upright
            - DRAPE_SIDE[c] * ((wing ? -ARM_REST : GATHER) * still * upright - WING_SPLAY * (1 - still)), DRAPE_SIDE[c] > 0 ? -inHold : -SWING_SIDE, DRAPE_SIDE[c] < 0 ? inHold : SWING_SIDE);
        springRings(motion, 1 + c * DRAPE_RINGS, tx, tz, dt);
        if (c === Math.floor(cols / 2))
            springRings(motion, 0, tx, tz, dt);
    }
    motion.draped = true;
    ...
    for (let c = 0; c < cols; c++) {
        ...
        for (let r = 0; r < DRAPE_RINGS; r++) {
            ...
            const z = clamp(motion.drapeZ[i], DRAPE_SIDE[c] > 0 ? -zin : -SWING_SIDE, DRAPE_SIDE[c] < 0 ? zin : SWING_SIDE);
 ...
 */
function springRings(motion, base, tx, tz, dt) {
...
const INTACT = Object.freeze({ lean: 0, spin: 0, crumple: 0, halved: false });
function newGore(built, field) {
    return {
        built, field, cut: null, piece: null, done: false,
        dropped: new Set(), shape: INTACT,
 ...
 */
function shapeOf(seam) {
    switch (seam) {
        // He falls the way the missing leg was holding him up from.
        case "hipL":
        case "kneeL": return { lean: 1, spin: 0.25, crumple: 0.3, halved: false };
        case "hipR":
        case "kneeR": return { lean: -1, spin: -0.25, crumple: 0.3, halved: false };
        // Over onto the side that still has weight on it, twisting as it goes.
        case "shoulderR":
        case "elbowR": return { lean: 0.5, spin: 1, crumple: 0, halved: false };
        case "shoulderL":
        case "elbowL": return { lean: -0.5, spin: -1, crumple: 0, halved: false };
 ...
 */
function beginGore(rig, motion, player, hooks) {
    const g = rig.gore;
    if (g.done)
        return;
    g.done = true;
    ...
    _gv.set(motion.hitSide * cy - motion.hitFwd * sy, 0, motion.hitSide * sy + motion.hitFwd * cy);
    if (_gv.lengthSq() < 1e-6)
        _gv.set(sy, 0, cy);
    _gv.normalize();
 ...
 */
function groundUnder(gore, x, z, at) {
    if (at)
        return at(x, z);
    if (Math.abs(x - gore.gx) < 0.5 && Math.abs(z - gore.gz) < 0.5)
        return gore.gy;
    gore.gx = x;
    gore.gz = z;
...
function setTopple(pitch, roll, yaw) {
...
function deathLayer(d, fall, shape, seed, cause, heavy, halfWidth) {
     ...
     */
    const jolt = (at, ring, damp) => D > at ? Math.exp(-damp * (D - at)) * Math.sin((D - at) * ring) * c.drive : 0;
    //
 ...
 */
function halfLayer(buckle, bounce, fall, d) {
 ...
 */
export function carryGore(rig, dx, dz) {
...
export function poseWarrior(rig, motion, player, dt, ctx, hooks) {
    ...
    const st = (() => {
        ...
        if (twoHander)
            return { ...body, rest: STANCE.berserker.rest, live: w.live, slide: w.slide, plumb: STANCE.berserker.plumb };
        if (rig.weaponClass === rig.warriorClass)
            return body;
    ...
    if (rig.mirror.scale.x !== wantMirror)
        rig.mirror.scale.x = wantMirror;
    if (player.id === ctx.localId)
        reportHand(rig, wantMirror);
    ...
    if ((dead || floored) && motion.actT <= dt)
        motion.fall = motion.hitFwd >= 0 ? 1 : -1;
    ...
    if (dead) {
        ...
        deathLayer(motion.actT, motion.fall, rig.gore.shape, motion.seed, player.deathCause, !!player.deathHeavy, Math.abs(piv.rightArm.position.x));
        stops();
    ...
    if (calm > 0.001)
        idleLayer(t, motion.seed, wounded, calm);
    ...
    if (motion.wMove > 0.001)
        gaitLayer(motion, Math.max(spd, 1.4), legLen, dt, motion.wMove, gaitArms);
    ...
    if (rig.shield) {
        ...
        if (!rig.gore.dropped.has(rig.shield))
            rig.shield.visible = carried;
        const wear = typeof boards === "number" ? 1 - boards / SHIELD.max : 0;
        for (const c of rig.shield.children) {
            if (!c.name.startsWith("crack"))
                continue;
            c.visible = carried && wear > CRACK_AT[Number(c.name.slice(5))];
        }
    }
    if (motion.wAction > 0.001)
        attackLayer(player.attackDir, swing, motion.heavy, carried, motion.wAction, player.comboCount);
    ...
    if (staggered) {
        ...
        staggerLayer(t, motion.actT / Math.max(0.2, motion.actT + player.staggerTimer), motion, smooth(clamp01(motion.actT / 0.09)));
    }
    if (rolling)
        dodgeLayer(motion.actT / 0.34, Math.sin(velAngle - motion.yaw), smooth(clamp01(motion.actT / 0.07)));
    if (casting)
        abilityLayer(motion.actT, 0.8);
    if (motion.flinch > 0.001 && !staggered)
        flinchLayer(motion, 1 - motion.wAction * 0.45);
...
function commit(rig, piv, st, blend, ready) {
    if (blend > 0) {
        const k = smooth(blend);
        for (const c of CHANNELS)
            P[c] = mix(P[c], rig.last[c], k);
    }
    Object.assign(rig.last, P);
    applyPose(rig, piv, st, ready);
}
function applyPose(rig, piv, st, ready) {
    const body = rig.body;
    ...
    if (rig.shield && !gone.has(rig.shield)) {
        ...
        // gyro-stabilised.
        rig.shield.rotation.set(-(P.olx + P.olb) * 0.94 + SHIELD_LEAN + P.sx, mix(SHIELD_REST_YAW, SHIELD_GUARD_YAW, ready) + P.sy, SHIELD_ROLL + P.sz);
        ...
        rig.shield.position.set(rig.offGrip.x - GRIP.x, rig.offGrip.y - GRIP.y + P.sfy, rig.offGrip.z - GRIP.z + P.sfz);
    }
}
/**
