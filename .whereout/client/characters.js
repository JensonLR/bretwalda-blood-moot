 ...
 *                inside — and pushed out onto `seatR` as it passes the collar,
 *                so it LIES ON the mail instead of through it.
 *   0.93 -> 1    THE TUCK. The inside wall comes back and dies INTO the throat,
 *                60 mm under the jawline, at negative lift. That is the hard
 *                straight cut at the neck, deleted: there is no free edge there
 *                any more, so there is no line to see.
 *
...
function beardShell(K, y0, nu, cut) {
    ...
    const at = (t, s, inset, out) => {
        ...
        if (q <= FACE) {
            ...
            o = oo * m * ridge;
            d = dd * m * reach(u) * cut.rag(u);
            // The last station comes back and dies in the throat, 60 mm under the
            // jawline. That is the "hard straight cut at the neck" deleted: there is
            // no free edge left there to cut with.
        ...
        // ---- IT SITS ON THE MAIL, IT DOES NOT PASS THROUGH IT ----
        //
...
export function buildCharacter(cls, ap, accents, materials, detail = "high", seed) {
    ...
    const torsoMeshes = emit("torso", root, () => {
        ...
        if (wallman) {
            ...
            p.add(shell(wear(layer([collar - 0.012, ramp, S.shoulderY + 0.02, S.chestY, S.waistY, S.hipY, mailHem + 0.05, mailHem], 0.036, [-0.004, 0, 0, 0, 0.004, 0.012, 0.036, 0.052]), 2.3), seg, { power: 2.3, wall: 0.016 }), robed ? buff : mail);
            if (heavy) {
                ...
                p.add(shell(wear(layer([collar, ramp, S.shoulderY + 0.015, S.chestY + 0.005], 0.05, [-0.008, 0, 0, 0.018]), 2.2), seg, { power: 2.2, wall: 0.014 }), mail);
            }
        }
    ...
    const hairCeil = (u, v) => {
        ...
        if (helmed && style.cheek !== "none"
            ...
            && v < bandLo && v > cheekHemAt(awayFromFace(u)) - 0.14)
    ...
    const coifSquash = (out) => {
        ...
        if (Math.abs(Math.atan2(px, pz)) < coifRim(L.v))
            return;
    ...
    const hairFall = (u) => {
        ...
        if (coifed)
            return 1 - smooth(coifRim(0) - 0.06, coifRim(0) + 0.28, awayFromFace(u));
    ...
    emit("head", headPivot, () => {
        ...
        if (ap.hairStyle !== "shaved") {
            ...
            if (ap.hairStyle === "braids") {
                ...
                for (const s2 of [-1, 1]) {
                    ...
                    if (hooded)
                        continue;
                    if (coifed && awayFromFace(rootU) > coifRim(0) - 0.10)
                        continue;
                    ...
                    const bp = (t, out) => {
                        ...
                        out.set(bRoot.x + s2 * (swingOut * swing), bRoot.y - drop * t, bRoot.z + swingFwd * swing);
        ...
        if (ap.beardStyle !== "none") {
            ...
            const hank = (u) => 0.17 * Math.cos(u * 7.3 + 0.4) + 0.10 * Math.cos(u * 12.9 - 1.1);
            ...
            const cuts = {
                // CLOSE CROP, free. A jawline beard with a clipped moustache and 44 mm
...
export function beardSeatProbe(cls, seed, beardStyle) {
    ...
    if (!nV)
        return { pieces: 0, throughMm: 0, worst: `${cls}/${beardStyle}`, overMm: 0, fallMm: 0 };
