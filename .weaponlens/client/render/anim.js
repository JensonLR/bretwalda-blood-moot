...
import { WARRIOR_STATS, SWING_PHASES, SHOVE, KNOCKDOWN, EMOTE_SECONDS } from "../../types.js";
import { buildCharacter, buildWeaponForClass, buildOffhandFor, buildShield, shieldBoard, peopleOf, defaultAppearance, ELBOW_ALONG, KNEE_ALONG, GRIP_ALONG, GRIP_PITCH, } from "../characters.js";
 ...
 */
function hangCloak(pivot, base, bound) {
    let lowest = 0;
    let half = 0;
    let deep = 0;
...
function attackLayer(dir, ph, heavy, shielded, w) {
...
export function poseWarrior(rig, motion, player, dt, ctx, hooks) {
    ...
    if (motion.wAction > 0.001)
        attackLayer(player.attackDir, swing, motion.heavy, !!rig.shield, motion.wAction);
    if (motion.wBlock > 0.001)
        blockLayer(!!rig.shield, clamp01(player.blockTimer / 0.22), motion.wBlock);
    if (shoving)
        shoveLayer(clamp01(motion.actT / (SHOVE.windup + SHOVE.recover)), !!rig.shield, smooth(clamp01(motion.actT / 0.06)));
    ...
    if (motion.emote) {
        if (bodyOwned || staggered) {
            ...
            if (ph >= 1)
                ...
                emoteLayer(motion.emote, ph, !!rig.shield);
        }
    }
    if (staggered) {
