...
import { getFeel, getHandedness, lockFootMark, lockReticle, lockView, routeLook, subscribeHandedness, } from "../input.js";
import { LAYER_UNOCCLUDED } from "./quality.js";
...
export function noteDrawnBody(id, body) {
    if (body)
        drawnBodies.set(id, body);
    else
        drawnBodies.delete(id);
}
/**
 ...
 */
function shoulderSign() {
    return getHandedness() ? -1 : 1;
}
export function createCameraRig(settings, opts = {}) {
    ...
    let photoFraming = null;
    let summaryShot = null;
    ...
    function follow(dt, ctx) {
        if (pendingAdopt)
            adoptSpawnHeading(ctx);
        ...
        aimAt(ctx.focus.x + fwdX * LOOK_AHEAD, LOOK_HEIGHT + bob * 0.7, ctx.focus.z + fwdZ * LOOK_AHEAD);
        snapNext = false;
        followFrame++;
        focusX = ctx.focus.x;
        focusZ = ctx.focus.z;
     ...
     */
    function orbit(dt, radius, height, spin, lerp, lookY, cx = 0, cz = 0) {
        yaw += dt * spin;
    ...
    const unsubscribeHand = subscribeHandedness(() => { });
    const rig = {
        camera,
        get yaw() {
            return yaw;
        ...
        setOccluders(solids, bound) {
            occluders = solids;
            occlBound = Number.isFinite(bound) ? bound : Infinity;
            boom = CAM_DIST;
        },
        ...
        dispose() {
            ...
            if (typeof window !== "undefined") {
                delete window.__bretwaldaCamera;
            }
