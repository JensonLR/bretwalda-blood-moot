...
const tri = (x) => Math.abs(2 * (x - Math.floor(x + 0.5)));
/**
 ...
 */
const contrast = (x, k) => clamp01((x - 0.5) * k + 0.5);
...
function valueOctave(out, size, cells, amp, rng) {
 ...
 */
function valueNoise(size, cells, octaves, gain, rng) {
...
function ridgedNoise(size, cells, octaves, rng) {
    const data = new Float32Array(size * size);
...
const sampleField = (f, u, v) => tap(f.data, f.size, u, v);
const sampleCell = (f, u, v) => tap(f.dist, f.size, u, v);
/** Nearest cell id — deliberately not interpolated, it is an identifier. */
function sampleCellId(f, u, v) {
    const s = f.size;
    const mask = s - 1;
    return f.id[(((v * s) | 0) & mask) * s + (((u * s) | 0) & mask)];
}
function createBank(size, seed) {
 ...
 */
function warpUV(bank, u, v, amp) {
    const w = (sampleField(bank.warp, u + 4, v + 4) - 0.5) * amp;
    WARP[0] = u + w;
    WARP[1] = v - w * 0.74;
}
const col = (hex) => [((hex >> 16) & 255) / 255, ((hex >> 8) & 255) / 255, (hex & 255) / 255];
function set(c, i, a) {
    const j = i * 3;
    c[j] = a[0];
    c[j + 1] = a[1];
...
function mix(c, i, a, b, t) {
    const j = i * 3;
    c[j] = a[0] + (b[0] - a[0]) * t;
    c[j + 1] = a[1] + (b[1] - a[1]) * t;
    c[j + 2] = a[2] + (b[2] - a[2]) * t;
}
function toward(c, i, b, t) {
    if (t <= 0)
        return;
    const j = i * 3;
    c[j] += (b[0] - c[j]) * t;
 ...
 */
function deriveAo(h, size, strength) {
 ...
 */
function bandLimit(src, size, out = new Float32Array(src.length)) {
    const n = size * size;
 ...
 */
function slopeVariance(h, hLo, size, strength) {
    const n = size * size;
    const d = scratch(1, n);
    for (let i = 0; i < n; i++)
        d[i] = h[i] - hLo[i];
 ...
 */
function specularAA(roughness, variance) {
 ...
 */
function mipRoughness(roughness, t) {
 ...
 */
function ormMips(size, ao, rough, metal, normal, 
 ...
 */
function deriveNormal(h, size, strength) {
...
function verdigris(c, i, light, dark, amount, mottle) {
    if (amount <= 0)
        return;
    const j = i * 3;
...
function buildLinen(g) {
    ...
    forEachTexel(size, (i, u, v) => {
        ...
        const shape = (s) => s * Math.sqrt(s); // crisp, thin, round-shouldered
...
function wood(g, o) {
    ...
    for (let i = 0; i < o.knots; i++)
        knots.push([rng(), rng(), 0.05 + rng() * 0.045]);
...
function buildMud(g) {
    ...
    const MUD = [water, wetMud, damp, drying, crust];
    forEachTexel(size, (i, u, v) => {
        warpUV(bank, u, v, 0.05);
        const wu = WARP[0];
        const wv = WARP[1];
...
function buildGrass(g) {
    ...
    const BLADE = [lush, midGreen, dryBlade, deadStraw];
 ...
 */
export function __probeSubstance(name, size) {
...
__probeSubstance.declared = (name) => RECIPES[resolve(name)].roughness;
const ALIAS = {
    ground: "dirt",
    wood: "oak",
    stone: "granite",
...
export function createTextureLibrary(renderer, settings) {
    ...
    function build(name) {
        ...
        if (packOrm) {
            ...
            for (let i = 0; i < texels; i++)
                rough[i] = specularAA(clamp01(gen.r[i]), sigma[i]);
    ...
    return {
        ...
        dispose() {
            ...
            for (const entry of surfaces.values()) {
                for (const tex of entry.owned)
                    tex.dispose();
            }
            for (const tex of sprites.values())
                tex.dispose();
            clones.clear();
            surfaces.clear();
            sprites.clear();
