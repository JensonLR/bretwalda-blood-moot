...
| 5.15 | **The helmet flank gap is contested space, not a loose constant** | **STILL OPEN — fifth attempt made, measured, REVERTED, and the row's own title was right all along.** I closed the gap and wrote in commit `d44c44a` that "the title of this row was wrong; it was not contested space". It is contested space, and `cosmetictest` said so an hour later. **Deriving the rim from the guard takes the Braided War-locks from 7.04% of silhouette under the Jarl's Crowned to 0.80% — an 89% loss on a 100g paid cosmetic** — with the Spectacle 7.29 -> 0.87 and the Boar-Crest 6.90 -> 0.78, and the Long Mane roughly halved on all three. **THE BRAIDS' ENTIRE VISIBILITY IS THAT GAP.** There is no knee on the curve: at the gentlest reach measured (K=0.35, window 6.0% -> 5.5%, an 8% gain) the braids are already down to 1.40%. Buying 28% of the owner's gap costs 89% of what a player paid for. That is a design call and not mine to make. **THE INSTRUMENT IS THE LESSON AND IT WAS ALREADY WRITTEN DOWN.** helmclash §5 stayed green through all of it, because it measures hair OUTSIDE metal and cannot see hair DELETED; `wearmeasure` §4's own header records this exact trap — *"a ratio whose denominator moves with its numerator measures nothing"* — and names the tool that catches it, `cosmetictest`'s silhouette against a fixed reference through a camera. It caught the head stack's first landing the same way. I ran the diff-the-red-rows discipline on the wrong ruler and called it settled. **WHAT SURVIVED, AND IT IS WORTH THE ATTEMPT:** the mail's top ring was 11 mm outside a low-passed form the skin stands 16 mm proud of, so the mail sat INSIDE the face across the crown and the ceiling floored at its 2 mm, leaving the hairline outside metal buried in the scalp — 3.87% out by 4.4 mm at az 88 deg on every rung with `hair=shaved` at 0.00, which is what says garment and not hairstyle. That is fixed and shipped separately. **WHAT THE NEXT ATTEMPT NEEDS:** not a rim, and not a plate. The gap has to be filled with something that is not opaque to the hair behind it, or the hair has to be re-rooted so its silhouette does not live in the one arc the mail wants. Judge it on `cosmetictest`, not on helmclash. |
