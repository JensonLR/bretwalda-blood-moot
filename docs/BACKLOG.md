...
| 0.5 | Wyrm-Crest's deep cheek guard, spread 28.8 vs Spectacle's 4.2 | **RE-OPENED 31 Aug 2026 — THE 24 Aug CLOSURE USED THE WRONG INSTRUMENT.** Re-measured on today's HEAD with the tool that raised it, `facelook --cover`: **23.4% / 50.5% / 53.1% of the lens-facing face taken at 0/-35/+35 deg, spread 29.7**, 95% of the loss `rig:head · 6e767f`, against the Spectacle's 4.2. The row was raised on 24.3/50.9/53.1 spread 28.8 — **nothing has moved in a week.** The closing evidence below cites a `helmclash` run, and helmclash CANNOT PRODUCE THAT QUANTITY: it has no camera, no lens and no bearings — its own header says every number is "taken off the triangles `buildCharacter` actually emits, in the head pivot's own frame", and the only thing in it called "spread" is a sorted distribution of millimetres at :2133. Two different questions, one word. `art/look/look.png` shows what the number means: at three-quarter the guard is a flat slab across the whole side of the face. See 5.16 for the reshape. The stale closure: | A fresh `helmclash` run carries FOUR wyrm rows and none is red: berserker at spread 5.6 (against the 28.8 this row was raised on, beside the Spectacle's 4.2), the runekeeper's plaits under the bar since the wearsweep scopings drove them back. §5 stands at its ledgered 77 of 288 — the exact standing state OPEN-DEFECTS records, no regression. |
...
| 4.8b | **THE FIRST MOOT — a cinematic starter tutorial for new arrivals** that teaches the fight AND takes the kingdom oath inside one flow. Owner-ruled replacement for the campaign (above). The bones already exist: `tuition.mjs` owns contextual teaching, solo rooms own a private fight with bots, the oath flow owns kingdom choice, and `deathcam`/`summary` own the cinematic grammar — this unit composes them into a first-run rite: arrive unsworn → a staged first fight that teaches strike/guard/shove/dodge in beats → the four peoples presented in their own grounds' colours → the oath taken → delivered to FIND A FIGHT | NEW — the design note lives in `docs/ARMOURY-REVIEW-PLAN.md`'s continuation list until it earns its own doc |
...
| 5.16 | **The Wyrm's cheek guard is a billboard, not a cheek piece** | **RE-OPENED AND ATTEMPTED 31 Aug 2026 — reverted, and the attempt is the useful part.** Walking `cheekIn` back 0.56 -> 0.95 takes the face from 23.4/50.5/53.1 to 12.6/27.4/29.3 (parity with the Spectacle at three-quarter) and reads correctly in `art/look/wyrm-after.png` — and costs `wearmeasure` §4 (29/30, braids outside the guard on the berserker) and a sixth flank window; at 0.80 the braids still fail and windows go to SEVEN. The guard's width is covering those braids. Together with 5.13's remainder and 5.15 this is ONE problem — plate and hair competing for the same arcs — and `docs/OPEN-DEFECTS.md` carries the synthesis. Original: **RE-OPENED 31 Aug 2026** (see 0.5 for why the 24 Aug closure did not hold). Measured on HEAD: face taken 23.4 / 50.5 / 53.1 % at 0/-35/+35 deg, spread 29.7 against the Spectacle's 4.2, 95% of it one surface. `cheekHemAt`'s deep branch is `lat(Y_CHIN + 0.05) + 0.34 * smooth(0.30, 1, t)^1.35` — a monotone ramp whose first THIRD is flat at chin height, which is the straight bottom edge you can see in the render. **The hem is not the lever, and that is already measured**: the recorded sweep moved that addend 0.34 -> 0.12 for a spread change of 28.8 -> 29.0, inert. What projects across the face at three-quarter is the plate's STANDOFF (21-25 mm over an open helm, `helmWear` block), and the budget under it is tiny and documented — `CHEEK_LINER` was already cut 5 mm -> 3 mm because the deep guard leaves the hair beneath it 1.5-3.1 mm. Same family as 5.15 and 5.13's remainder: the head-stack reshape, with `maskBot(u)` — "the one piece in the tier the audit passes" — as the template. |
| 5.15 | **The helmet flank gap is contested space, not a loose constant** | **NEW 31 Aug 2026 — third attempt made, measured, reverted.** Deriving the coif's rim from the guard's rear edge through one definition CLOSES the owner's gap (five §10 windows -> three, Spectacle and Boar-Crest outright, Crowned 6.0% -> 3.3%, all eleven wearmeasure sections still PASS) and costs **WRAP 6 -> 13 and PELT 74 -> 91** on the hooded huscarl. The gap is where his hair comes out; filling it with mail displaces the hair, which is why the two earlier attempts were reverted from the plate's side. Done means the head-stack reshape, not another constant. The new helmclash baseline gate makes the next attempt cheap to judge. |
