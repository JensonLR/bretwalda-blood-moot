...
| 4.9 | **Other game modes** | **RULED 26 Aug 2026 — TWO modes:** the TOURNAMENT MOOT (bracketed 1v1s for 4-8 on `honour_duel`, winners advance, the hall watches the final — wants spectate, see the clips/spectate ruling) and the HORDE mode — co-op last stand for a WAR PARTY against escalating waves of the *here* (the Chronicle's own word for the raiding host). Owner: "maybe find a cooler Anglo Saxon ish name" than Shield Wall — working name **THE BURH** (Alfred's defended forts; you hold the burh against the here), to be put to him with alternatives before it ships. |
...
| 7.3 | **The Tournament Moot (4.9a)** | Bracketed 1v1s, 4-8 men, on `honour_duel`; the hall watches the final. **Sequenced after 7.9's spectate seats by the owner's own ruling** ("Both, clips first... spectate lands with the Tournament Moot which needs it anyway") — building the bracket before the waiting men can watch would ship a tournament of men staring at a lobby. |
...
| 7.7 | **Fight depth: executions, weapon choice per class, directional guard** | All three ruled in. Executions build on gore + kill-cam; weapon choice gives each class 2-3 historically right weapons with real trade-offs; directional guard is gated on the 7.1 controls review landing first. |
