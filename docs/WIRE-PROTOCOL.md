...
| `rejoin` | `{code, key}` | a player in that room is AWOL with this `reconnectKey` | **THE WAY BACK IN (8.9).** A floor human whose SOCKET drops mid-match (any active phase; never the lobby, never a pressed LEAVE — that is a surrender, pronounced at once) is held for `AWOL_GRACE` (12 s sim): his body stands, no `player_left`, the muster still releases if it was waiting on him. Each join reply carries the caller's own `reconnectKey` top-level (a CREDENTIAL — on `PRIVATE_FIELDS`, never in any snapshot); a fresh session presenting it while the body is AWOL rebinds to the SAME player and gets a full join-shaped reply. Key AND awol, both: the key alone cannot hijack a live man, and the grace expiring removes the body through the departure's own door (round end, host handoff, room death — all exactly as a leave). A refusal is an `error`; the honest next move is a plain `join`, which seats the mead-bench. The client's transport retries the socket itself (three laps then the SSE fallback) and fires the rejoin on a synthetic `relink` event that never rides the wire. |
...
| `hold_bots` | `{hold}` | host only; the room is SOLO (`maxPlayers` 1) | The First Moot's pause (owner, 3 Sep 2026). Raises (`hold:true`) or drops (`hold:false`) the pell-hold on EVERY bot in the room — held bots stand and do not step, circle or strike. Honoured only in a room sealed to other humans, for the reason `arm_bots` has no way back: a hold that could reach a live opponent is a way to freeze him. The client sends `true` while a phase's card is up and `false` when the card is dismissed; `arm_bots` still governs the first arming (nobody swings before THE SHIELD). |
