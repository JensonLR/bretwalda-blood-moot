...
export function isTouchPrimary() {
    ...
    if (/iPhone|iPad|iPod|Android|Mobile/i.test(ua))
        return true;
    if (/Macintosh/.test(ua) && points > 1)
        return true; // iPadOS in its Mac disguise
    if (typeof window.matchMedia !== "function")
        return "ontouchstart" in window || points > 0;
...
const isTier = (v) => v === "high" || v === "medium" || v === "low";
const step = (t, by) => TIER_ORDER[Math.max(0, Math.min(TIER_ORDER.length - 1, TIER_ORDER.indexOf(t) + by))];
/**
 ...
 */
export function detectTier(probe = probeDevice()) {
    ...
    if (!probe.touch)
        return probe.width < 900 ? "medium" : "high";
...
export const QUALITY_CHOICES = ["high", "medium", "low", "auto"];
/** Menu labels, so two surfaces cannot drift into calling `high` two things. */
export const QUALITY_CHOICE_LABELS = {
    high: "High",
    medium: "Balanced",
    low: "Fast",
    auto: "Automatic",
 ...
 * blunt version.
 */
export function setQualityPreference(choice) {
 ...
 */
export function chooseQuality(choice) {
 ...
 */
export function readQualityOverride() {
    if (typeof window === "undefined")
        return null;
    const pinned = window.__quality;
    if (isTier(pinned))
        return pinned;
    try {
 ...
 */
let forgedTier = null;
export function resolveQuality(override) {
    ...
    if (typeof window !== "undefined") {
        try {
            ...
            if (Number.isInteger(c) && c >= 1 && c <= 4)
                settings.settlementShadowCadence = c;
            ...
            if (Number.isInteger(h) && h >= 1 && h <= 4)
                settings.hearthShadowCadence = h;
        }
        catch {
            /* malformed query string is not worth a crash */
        }
...
class QualityGovernor {
    ...
    arm(tier) {
        ...
        const optedIn = window.__governor === "measure";
        if (typeof navigator !== "undefined" && navigator.webdriver && !optedIn)
            return;
        this.running = true;
        this.last = 0;
    ...
    tick = (now) => {
        ...
        this.frames.push(dt);
        if (this.frames.length < GOVERNOR_WINDOW)
            return;
    ...
    judge() {
        ...
        if (this.p50 <= GOVERNOR_GOOD_P50_MS && this.p95 <= GOVERNOR_GOOD_P95_MS) {
            ...
            if (this.good < (reclaiming ? GOVERNOR_RECLAIM : GOVERNOR_AGREE))
                return;
            this.good = 0;
            this.tier = next;
            if (reclaiming)
    ...
    applyLive(tier) {
        ...
        if (Math.abs(r.getPixelRatio() - ratio) < 0.01)
            return;
...
export function setLayerDeep(root, layer) {
