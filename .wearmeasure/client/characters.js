...
function faceSurface(K, d, out) {
    ...
    const band = (u, k) => bump(u, 0, 0, mw * 1.9, 1, 1) * (1 - smooth(mw * k, mw * (k + 0.30), Math.abs(u)));
...
export function headProbe(cls, seed) {
    ...
    let tipHalf = 0;
    {
        ...
        for (let i = 0; i <= 240; i++) {
            ...
            if (p.z >= tip.z - 0.003)
                tipHalf = Math.max(tipHalf, Math.abs(p.x));
        }
    }
...
export function buildCharacter(cls, ap, accents, materials, detail = "high", seed) {
    ...
    const torsoMeshes = emit("torso", root, () => {
        ...
        if (!bare) {
            p.add(shell(layer([collar, ramp, S.shoulderY + 0.01, S.chestY, S.waistY, S.hipY, tunicHem + 0.06, tunicHem], 0.021, [-0.003, 0, 0, 0, 0.003, 0.01, 0.03, 0.045]), seg, { power: 2.3, wall: 0.014 }), robed ? cloakMat : wool);
        }
        else {
            ...
            // all limb.
            p.add(shell(layer([S.shoulderY + 0.02, S.chestY - 0.02, S.waistY, tunicHem], 0.024, [0, 0, 0.002, 0.008]), seg, { power: 2.3, wall: 0.016 }), buff);
            ...
            if (!bare) {
                ...
                for (let i = 0; i < lames; i++) {
                    ...
                    if (lod.trim) {
                        ...
                        p.add(ring(capAt(yB) * 1.02, 0.0062, 4, 12), lamellar ? steel : iron, xf(0, yB, 0, Math.PI / 2, 0, 0, 1, 1, 1.03));
                    }
                }
                if (lod.trim) {
                    for (let i = 0; i < 4; i++) {
    ...
    emit("head", headPivot, () => {
        ...
        for (const s of [-1, 1]) {
            ...
            const RAKE = 0.32;
            const ear = (ex, ey, ez, roll, kx, ky, kz) => new THREE.Matrix4()
            ...
            const arc = 4.9;
            p.add(new THREE.TorusGeometry(0.0235, 0.0048, 5, 14, arc), headSkin, new THREE.Matrix4()
                .makeTranslation(s * (R.x * 0.93), earY, -0.024)
                .multiply(new THREE.Matrix4().makeRotationY(s * Math.PI / 2))
                .multiply(new THREE.Matrix4().makeRotationZ(RAKE))
            ...
            if (lod.trim) {
                ...
                p.add(new THREE.TorusGeometry(0.0105, 0.0033, 4, 9, 3.2), headSkin, new THREE.Matrix4()
        ...
        if (ap.hairStyle !== "shaved") {
            ...
            if (ap.hairStyle === "long") {
                ...
                for (const s of [-1, 1]) {
                    ...
                    ], Math.max(9, lod.limb + 1), { power: 2.0, wall: 0.013 }), hair, xf(s * R.x * 0.44, 0, 0, 0, 0, -s * 0.05));
                }
                if (lod.trim) {
        ...
        if (ap.beardStyle !== "none" && ap.beardStyle !== "short") {
            ...
            if (!style.mask) {
                ...
                if (full) {
                    ...
                    // the corner.
                    const swell = (u, v) => 0.0010 + 0.0052 * Math.sin(Math.PI * clamp01(v))
            ...
            if (ap.beardStyle === "full") {
                ...
                const belly = (k, lean) => p.add(shell([
                ...
                ], Math.max(8, lod.limb), { power: 2.15, capTop: true, capBottom: true }), beard, xf(0, 0, 0, 0, 0, lean));
                belly(1, 0);
                if (lod.trim)
                    belly(0.94, 0.075);
            }
                ...
                for (const s of [-1, 1]) {
                    ...
                    ], Math.max(8, lod.limb), { power: 2.1, capBottom: true }), beard, xf(s * 0.030, 0, 0.004, 0.10, 0, -s * 0.34));
                }
            }
                ...
                p.add(braid((t, out) => out.set(0, skullY - 0.196 - 0.150 * t, 0.036 - 0.010 * t * t), { turns: 2.3, rows: Math.max(10, lod.limb * 2), ring: Math.max(5, lod.limb - 2),
                    radius: (t) => 0.0255 * (1 - 0.55 * t * t) }), beard);
                for (let i = 0; i < 2; i++) {
                    const t = 0.14 + i * 0.74;
                    p.add(ring(0.0245 - i * 0.010, 0.0044, 4, 10), brass, xf(0, skullY - 0.196 - 0.150 * t, 0.036 - 0.010 * t * t, Math.PI / 2, 0, 0));
                }
            }
        }
        ...
        if (helmed) {
            ...
            if (lod.trim) {
                ...
                for (let i = 0; i < 4; i++) {
                    ...
                    p.add(headWear(K, {
                        ...
                        lift: (_u, s) => 0.013 + (crest - 0.013) * s * s + 0.006 * (1 - Math.pow(clamp01(s), 2.2)),
                        thick: 0.004,
            ...
            if (style.crown === "ridge") {
                ...
                const section = (du) => Math.sqrt(Math.max(0, 1 - Math.pow(clamp01(Math.abs(du) / ridgeHalf), 2)));
            ...
            if (style.crown === "wyrm") {
                ...
                const wSection = (du) => Math.sqrt(Math.max(0, 1 - Math.pow(clamp01(Math.abs(du) / wyrmHalf), 2)));
                for (const u of [0, Math.PI]) {
                    p.add(headWear(K, {
                        u0: u - wyrmHalf, u1: u + wyrmHalf,
                        v0: () => (u === 0 ? bandLo - 0.04 : bandLo - 0.24), v1: () => Math.PI / 2 - 0.02,
                        nu: 5, nv: 5,
                        lift: (x, v) => 0.014 + (0.006 + 0.052 * Math.pow(Math.sin(Math.PI * clamp01(v * 0.84 + 0.10)), 0.55)) * wSection(x - u),
                        thick: 0.009,
                    }), trimMetal, place.clone());
                }
                ...
                if (lod.trim) {
                    for (const s of [-1, 1]) {
                        p.add(rod(0.0008, 0.0034, 0.026, 4), brass, xf(hx + s * 0.010, hy + 0.014, hz - 0.008, -0.75, 0, -s * 0.42));
