 ...
 */
function teamDye(hex, field, sat) {
    const hsl = { h: 0, s: 0, l: 0 };
 ...
 */
export function teamKit(kit, team) {
    if (team === "none")
        return kit;
    return {
 ...
 */
export function teamWorn(hex, team, kind) {
    if (team === "none")
        return hex;
 ...
 */
export function teamCloak(hex, team) {
 ...
 */
export function shieldBoard(ap, team) {
    if (team !== "none")
        return TEAM_FIELDS[team].board;
...
export const CLOAK_VALUES = Object.keys(CLOAK_CUTS);
/**
...
function shellPoint(c, y, az, out) {
    const k = 2 / c.power;
    const st = c.st(y);
    const cs = Math.cos(az);
    const sn = Math.sin(az);
    return out.set(st.hw * Math.sign(cs) * Math.pow(Math.abs(cs), k), y, (st.z ?? 0) + st.hd * Math.sign(sn) * Math.pow(Math.abs(sn), k));
}
 ...
 * list that inverts.
 */
function shellNormal(c, y, az, out) {
    const da = 0.02;
    const dy = 0.004;
 ...
 */
function azAtX(c, y, x, front = true) {
    const st = c.st(y);
...
function seatXf(c, y, az, lift, roll = 0, sx = 1, sy = 1, sz = 1) {
    ...
    const m = new THREE.Matrix4().makeBasis(_sxE1.clone().multiplyScalar(sx), _sxE2.clone().multiplyScalar(sy), _sxN.clone().multiplyScalar(sz));
 ...
 */
const padded = (c, d) => ({
    st: (y) => { const st = c.st(y); return { y, hw: st.hw + d, hd: st.hd + d, z: st.z }; },
    power: c.power,
});
/** A station list read at a height, clamped to its own ends. */
function stationAlong(sts, y) {
    let i = 0;
    while (i < sts.length - 2 && y < sts[i + 1].y)
        i++;
    const a = sts[i];
    const b = sts[i + 1];
 ...
 * to see that.
 */
function fitAdd(p, tag, c, geo, mat, m) {
    if (_fitSpy) {
 ...
 */
function auricle(K, earRootX, side, press) {
    const pos = [], uv = [];
 ...
 */
function earSeatRaise(K, nu, nv, pos) {
    const n = (nu + 1) * (nv + 1);
 ...
 *              file's word for it.
 */
export function neckProbe(cls, seed, ys) {
...
function neckReach(S, ST, dx, dy, dz) {
...
export function helmFitProbe(cls, seed, helm) {
    ...
    const kitMaskFrom = (base) => {
        ...
        for (let k = 0; k < m.length; k++)
            m[k] = aug.r[k] > base.r[k] + 0.002 ? 1 : 0;
        return m;
    };
 ...
 */
function globePatch(f, axes, radius, thick, nu, nv, extent, wrapU) {
 ...
 * was typed twice.
 */
function lidMarginPoint(f, upper, t, out) {
...
function addEyes(p, K, lod, place, M) {
    ...
    for (const side of [-1, 1]) {
        ...
        if (_eyeClipSpy) {
            ...
            const axisOf = (a) => a.clone().add(f.c).applyMatrix4(place).sub(f.c.clone().applyMatrix4(place)).toArray();
            const marginOf = (up) => {
                const o = [];
                const v = new THREE.Vector3();
                for (let i = 0; i <= 96; i++) {
 ...
 */
export function eyeClipProbe(cls, seed) {
    const spy = [];
    ...
    root.traverse((o) => {
        if (o.isMesh && o.name === `${RIG_TAG}head`)
            headWorld = o.matrixWorld;
    });
...
function beast(spine, opts) {
    ...
    for (let j = 0; j <= N; j++) {
        ...
        if (t < 0.99) {
            spine(Math.min(1, t + 0.012), ahead);
            tan.subVectors(ahead, c);
        }
        else {
            spine(t - 0.012, back);
            tan.subVectors(c, back);
        }
...
function braidProfile() {
    ...
    for (let i = 0; i <= 2 * M; i++) {
        ...
        let bind = 0;
        for (const b of BIND)
            bind += 0.0062 * Math.exp(-Math.pow((t - b) / 0.042, 2));
 ...
 */
function beardShell(K, y0, nu, cut) {
    const prof = cut.prof;
     ...
     */
    const mouth = (u, y) => clamp01((1 - smooth(0.24, 0.46, Math.abs(u)))
    ...
    const dens = (u, y) => {
        ...
        const hankF = 0.86 + 0.14 * (0.58 * Math.cos(u * 8.5 + 0.4)
 ...
 */
const reflectChirality = (geo) => reflectAxis(geo, "x");
const mirrorZ = (geo) => reflectAxis(geo, "z");
/**
...
function fistGeometry(lod, scale, opts) {
     ...
     */
    const centroidOf = (geo, into) => {
        const a = geo.getAttribute("position");
        into.set(0, 0, 0);
        for (let i = 0; i < a.count; i++)
            into.x += a.getX(i), into.y += a.getY(i), into.z += a.getZ(i);
    ...
    if (tipN)
        tipC.divideScalar(tipN);
    const mark = (x, y, z) => new THREE.Vector3(-x, y, opts.mirror ? -z : z);
    return {
 ...
 */
export function bodyFitProbe(cls, seed, cloak) {
    const spy = [];
    const prev = _fitSpy;
    _fitSpy = spy;
    try {
    ...
    for (const rec of spy) {
        ...
        if (!prevRow)
            out.set(rec.tag, { tag: rec.tag, standoffMm: stand, sinkMm: sink });
        else {
 ...
 */
export function backCarryProbe(cls, seed, cloak) {
    const back = [];
    const cs = { rec: null };
    const worn = { layers: [] };
        ...
        for (let j = 0; j <= NV; j++) {
            for (let i = 0; i <= NU; i++) {
                ...
                for (const w of worn.layers) {
                    if (q.y > w.sts[0].y + 1e-6 || q.y < w.sts[w.sts.length - 1].y - 1e-6)
                        continue;
                ...
                if (!best)
                    continue;
                const b = best;
                const carrier = { st: (yy) => stationAlong(b.sts, yy), power: b.power };
 ...
 */
export function handProbe(cls, seed) {
    const spy = [];
    const prev = _fistSpy;
    _fistSpy = spy;
    try {
 ...
 */
export function gazeProbe(cls, seed) {
    const spy = [];
    const prev = _eyeSpy;
    _eyeSpy = spy;
    try {
    ...
    if (spy.length !== 2)
        return { eyes: spy.length, converge: 0, fixation: 0, skew: 9, ipd: 0, miss: 999, ahead: -1, irisOffAxis: 999 };
    ...
    const mir = (v) => new THREE.Vector3(v.x * BODY_MIRROR_X, v.y, v.z);
...
export function buildAxe(materials) {
    ...
    for (const { geo, mat } of part.merge())
        roll.add(new THREE.Mesh(geo, mat));
    g.add(roll);
    return g;
}
/**
 ...
 */
export function buildShield(color = 0x6b4226, materials, armorColor = 0x5f6b7a, 
/**
...
function signatureOf(cls, ap, accents, detail, lib, team) {
    return [
...
export function hairFitProbe(cls, seed, helm, hairStyle) {
    ...
    for (let i = 0; i + 2 < hairPts.length; i += 3) {
        ...
        if (hairPts[i + 1] < coverLo[k] - 0.002) {
            shown++;
            continue;
        }
        if (!solid[k])
    ...
    const binW = (iv) => Math.cos(((iv + 0.5) / NV - 0.5) * Math.PI);
    ...
    for (let i = 0; i + 2 < hairPts.length; i += 3) {
        ...
        const k = iv * NU + iu;
        if (!Number.isFinite(inner[k]) || hairPts[i + 1] < coverLo[k] - 0.002)
            seenBin[k] = 1;
    }
    let bareW = 0, keptW = 0;
...
team = "none") {
    ...
    const flax = (girth) => thrifty ? wool : M.tinted("linen", teamWorn(0xc2b69c, team, "cloth"), { repeat: clothRepeat(girth) });
    ...
    const beardFade = (amt) => {
        ...
        const c = new THREE.Color(mix(base.r, whisk.r, amt) / Math.max(0.03, whisk.r), mix(base.g, whisk.g, amt) / Math.max(0.03, whisk.g), mix(base.b, whisk.b, amt) / Math.max(0.03, whisk.b));
        ...
        if (hi > BEARD_FADE_CAP)
            c.multiplyScalar(BEARD_FADE_CAP / hi);
        return c;
    };
    ...
    const pelt = (girth) => M.tinted("wool", teamWorn(0x8a7050, team, "leather"), { repeat: Math.max(1, Math.round(girth / PELT_TILE)) });
    ...
    const trunkStack = (bare || wallman) ? null
        : layer([collar - 0.012, ramp, S.shoulderY + 0.02, S.chestY, S.waistY, S.hipY,
            (heavy ? S.hemY - 0.03 : S.hemY + 0.26) + 0.05, heavy ? S.hemY - 0.03 : S.hemY + 0.26], 0.036, [-0.004, 0, 0, 0, 0.004, 0.012, 0.036, 0.052]);
    /**
    ...
    const torsoMeshes = emit("torso", root, () => {
        ...
        const outer = (y) => {
            ...
            let widest = -1;
            for (const w of worn) {
                if (y > w.sts[0].y + 1e-6 || y < w.sts[w.sts.length - 1].y - 1e-6)
                    continue;
            ...
            if (!best)
                return { st: (yy) => at(yy, 0), power: 2.4 };
            const b = best;
            return { st: (yy) => stationAlong(b.sts, yy), power: b.power };
        };
        ...
        if (!bare) {
            if (robed) {
                p.add(shell(wear(layer([collar, ramp, S.shoulderY + 0.01, S.chestY, S.waistY, S.hipY, tunicHem + 0.06, tunicHem], 0.021, [-0.003, 0, 0, 0, 0.003, 0.01, 0.03, 0.045]), 2.3), seg, { power: 2.3, wall: 0.014 }), cloakMat);
            }
            else {
                // Closed from the collar to where the slits start, just below the hip.
                p.add(shell(wear(layer([collar, ramp, S.shoulderY + 0.01, S.chestY, S.waistY, S.hipY, slitY], 0.021, [-0.003, 0, 0, 0, 0.003, 0.01, 0.022]), 2.3), seg, { power: 2.3, wall: 0.014 }), wool);
            ...
            const openGap = 0.30;
            p.add(shell(wear(layer([S.shoulderY + 0.02, S.chestY - 0.02, S.waistY, tunicHem], 0.024, [0, 0, 0.002, 0.008]), 2.3), seg, {
            ...
            if (lod.trim) {
                ...
                for (let i = 0; i < 4; i++) {
                    ...
                    const jerkin = { st: (yy) => at(yy, 0.024), power: 2.3 };
                    ...
                    for (let j = 0; j < rung; j++) {
                        ...
                        fitAdd(p, "jerkin-lace", jerkin, box(w * 1.12, 0.0075, 0.006), hide, seatXf(jerkin, y, azr, 0.003, (i % 2 ? 1 : -1) * 0.16));
                    }
                    for (const s of [-1, 1]) {
                        fitAdd(p, "jerkin-eyelet", jerkin, ring(0.0055, 0.0022, 4, 8), brass, seatXf(jerkin, y, Math.PI / 2 - s * openGap, 0.0022, 0, 1, 1, 0.6));
                    }
                }
            }
        }
        ...
        if (wallman) {
            ...
            p.add(shell(wear(layer([collar - 0.012, ramp, S.shoulderY + 0.02, S.chestY, S.waistY, S.hipY, byrnieHem + 0.04, byrnieHem], 0.034, [-0.004, 0, 0, 0, 0.004, 0.012, 0.026, 0.036]), 2.3), seg, { power: 2.3, wall: 0.016 }), mail);
            ...
            if (lod.trim) {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    fitAdd(p, "doubling-rivet", dbl, ball(0.0075, 6), brass, seatXf(dbl, S.shoulderY - 0.012, a, 0.0045, 0, 1, 1, 0.6));
                }
            }
            ...
            p.add(shell(wear(layer([collar - 0.012, ramp, S.shoulderY + 0.028], 0.03, [-0.004, 0, 0.008]), 2.3), seg, { power: 2.3, wall: 0.013 }), mail);
        }
        else if (!bare) {
            ...
            p.add(shell(wear(trunkStack, 2.3), seg, { power: 2.3, wall: 0.016 }), robed ? buff : mail);
            if (heavy) {
                ...
                p.add(shell(wear(shoulderStack, 2.2), seg, { power: 2.2, wall: 0.014 }), mail);
            }
        }
        ...
        const beltC = { st: (y) => stationAlong(beltSts, y), power: 2.3 };
        ...
        for (const s of [-1, 1]) {
            fitAdd(p, "belt-buckle", beltC, box(0.006, bkY * 2 + 0.011, 0.010), brass, seatXf(beltC, S.beltY, azAtX(beltC, S.beltY, s * bkX), 0.005));
        }
        for (const s of [-1, 1]) {
            fitAdd(p, "belt-buckle", beltC, box(bkX * 2 + 0.006, 0.0055, 0.010), brass, seatXf(beltC, S.beltY + s * bkY, front, 0.005));
        }
        fitAdd(p, "belt-buckle", beltC, box(0.005, bkY * 1.7, 0.007), brass, seatXf(beltC, S.beltY, front, 0.009));
        fitAdd(p, "strap-end", beltC, box(0.026, 0.11, 0.01), hide, seatXf(beltC, S.beltY - 0.05, azAtX(beltC, S.beltY - 0.05, 0.055), 0.005, -0.12));
        if (lod.trim) {
        ...
        if (!robed) {
            ...
            for (let i = 0; i < runs; i++) {
                ...
                for (const face of [1, -1]) {
                    ...
                    fitAdd(p, "baldric", carry, box(0.046, ((yTop - yBot) / runs) * 1.6, 0.013), buff, seatXf(carry, y, azAtX(carry, y, x, face > 0), 0.0065, 0.4));
                }
            }
            ...
            // a fitting OF now, which is itself seated on the garment.
            {
                ...
                fitAdd(p, "baldric-boss", strap, ball(0.017, 8), brass, seatXf(strap, bossY, azAtX(strap, bossY, -0.125), 0.0090, 0, 1, 1, 0.55));
            }
        }
        ...
        if (bare) {
            ...
            p.add(shell(wear(layer([collar - 0.006, ramp, S.shoulderY + 0.03, S.chestY + 0.02], 0.055, [-0.03, -0.012, 0.025, 0]), 2.0), seg, { power: 2.0, wall: 0.02 }), fur);
            if (lod.trim) {
            ...
            const peltLoft = (v) => PELT_LOFT * (0.55 + 0.45 * v * v);
            const peltSurf = (u, v, lift, out) => {
            ...
            if (_backSpy) {
                const src = peltGeo.getAttribute("position");
                const pts = [];
                for (let i = 0; i < src.count; i++)
                    pts.push(new THREE.Vector3(src.getX(i), src.getY(i), src.getZ(i)));
                _backSpy.push({ tag: "back-pelt", carrier: peltOn, pts });
            }
            p.add(peltGeo, furPelt);
            for (let i = 0; i < 4; i++) {
                const boneY = S.chestY + 0.06;
                const chest = { st: (y) => at(y, 0), power: 2.4 };
                fitAdd(p, "bone-string", chest, rod(0.008, 0.003, 0.06, 5), M.tinted("bone", 0xd8cfb4, { repeat: 1 }), seatXf(chest, boneY, azAtX(chest, boneY, -0.06 + i * 0.04), 0.010, 0.2 - i * 0.13)
        ...
        if (robed) {
            ...
            p.add(shell(wear(layer([collar - 0.004, ramp, S.shoulderY + 0.018, S.chestY - 0.01, S.chestY - 0.055], 0.036, [-0.004, 0, 0.004, 0.016, 0.026]), 2.2), seg, { power: 2.2, wall: 0.013 }), wool);
            if (lod.trim) {
                ...
                for (const s of [-1, 1]) {
                    ...
                    fitAdd(p, "toggle", mant, rod(0.0055, 0.0042, 0.026, 6), M.tinted("bone", 0xd8cfb4, { repeat: 1 }), seatXf(mant, togY, azAtX(mant, togY, s * 0.034), 0.0055, s * 0.5)
        ...
        if (heavy && lod.trim) {
            ...
            for (let i = 0; i < 5; i++) {
                fitAdd(p, "mantle-boss", mantle, ball(0.014, 6), brass, seatXf(mantle, bossY, Math.PI / 2 - (-0.7 + i * 0.35), 0.009));
            }
        }
        if (wallman) {
        ...
        if (ap.cloak !== "none") {
            ...
            if (clasp === "pin") {
                ...
                fitAdd(p, "cloak-clasp", pinned, ball(0.0085, 8), hide, seatXf(pinned, cy, az, 0.006, 0, 1, 1, 0.7).multiply(xf(0.012, 0.023, 0)));
            }
            else if (clasp === "disc") {
                fitAdd(p, "cloak-clasp", pinned, ball(0.021, 10), brass, seatXf(pinned, cy, az, 0.0116, 0, 1, 1, 0.55));
                ...
                fitAdd(p, "cloak-clasp", pinned, ring(0.030, 0.0055, 5, 14), brass, seatXf(pinned, cy + 0.004, az, 0.0055));
                fitAdd(p, "cloak-clasp", pinned, shell([
                    { y: -0.046, hw: 0.0028, hd: 0.0028 },
                    { y: 0.030, hw: 0.0052, hd: 0.0052 },
                ], 6, { capTop: true, capBottom: true }), brass, seatXf(pinned, cy + 0.004, az, 0.0105, -0.62));
            }
            else {
                ...
                fitAdd(p, "cloak-clasp", pinned, ring(0.036, 0.0075, 5, 16), gilt, seatXf(pinned, cy, az, 0.0075));
                fitAdd(p, "cloak-clasp", pinned, ball(0.030, 12), gilt, seatXf(pinned, cy, az, 0.0120, 0, 1, 1, 0.40));
                fitAdd(p, "cloak-clasp", pinned, ball(0.011, 8), gilt, seatXf(pinned, cy, az, 0.0220, 0, 1, 1, 0.8));
            }
        }
        ...
        emit(`arm${side}`, pivot, () => {
            ...
            if (!bare) {
                ...
                if (lod.trim) {
                    ...
                    const capC = {
                        st: (y) => ({ y, hw: capAt(y) * 1.015, hd: capAt(y) * 1.045 }),
                        power: 2.2,
                    };
                    for (let i = 0; i < 4; i++) {
                        const a = -0.9 + i * 0.6;
                        fitAdd(p, "cap-rivet", capC, ball(0.008, 6), brass, seatXf(capC, 0.022, Math.PI / 2 - a, 0.003));
                    }
                }
                if (heavy || wallman) {
                    p.add(shell([
            ...
            const bracerC = { st: (y) => stationAlong(bracerSts, y), power: 2 };
            const outboard = side > 0 ? 0 : Math.PI;
            if (lod.trim) {
                for (let i = 0; i < 3; i++) {
                    const y = mix(elbow - 0.07, wrist + 0.01, (i + 0.5) / 3);
                    fitAdd(p, "bracer-buckle", bracerC, box(0.014, 0.018, 0.008), brass, seatXf(bracerC, y, outboard, 0.004));
                }
            ...
            if (robed) {
                fitAdd(p, "wrist-rune", bracerC, box(0.006, 0.05, 0.008), rune, seatXf(bracerC, wrist + 0.07, outboard, 0.004));
            }
            ...
            if (_fistSpy)
                _fistSpy.push({ side, shoulderX: side * S.shoulderX, marks: fist.marks, place: hand.clone() });
     ...
     */
    const napeHemY = Math.max(S.neckRoot - S.neckTop + 0.025, skullY - R.y * (style.nape === "guard" ? 1.12 : 0.45));
    // ---- where the neck is ----
    //
    ...
    const NECK_STATIONS = neckStations(S);
    const neckSection = (y) => neckSectionAt(NECK_STATIONS, y);
    /**
     * The neck's REARMOST skin, from the head's axis, in the HEAD PIVOT's frame.
     *
     ...
     */
    const neckBackAt = (y) => neckBackFrom(S, NECK_STATIONS, y);
    /**
    ...
    const napeHalf = (v) => Math.max(style.nape === "guard" ? 1.30 : 1.08, napeLap)
     ...
     */
    const coifSquash = (out) => {
        if (!coifed || out.y < coifLevels[coifLevels.length - 1].y)
            return;
        const L = coifRingAt(out.y);
        const px = out.x / L.hw, pz = (out.z - L.z) / L.hd;
        if (!style.mask && Math.abs(Math.atan2(px, pz)) < coifRim(L.v))
            return;
     ...
     */
    const shoulderRide = (out) => {
        if (!coifed || out.y >= coifHemY)
            return;
     ...
     */
    const gatherFit = (out) => fitFall(out, true);
    const fallFit = (out) => fitFall(out, false);
    /**
    ...
    const hairFall = (u) => {
        ...
        if (coifed)
            return smooth(2.26, 2.70, awayFromFace(u));
    ...
    emit("head", headPivot, () => {
        ...
        for (const s of [-1, 1]) {
            ...
            const A = auricle(K, earRootX, s, style.mask || style.cheek === "deep" ? EAR_PRESS : 0);
        ...
        if (ap.hairStyle !== "shaved") {
                    ...
                    for (let i = 0; i < N; i++) {
                        ...
                        if (Number.isFinite(room)) {
                            if (room <= 0) {
                                ...
                                if (sink < 0)
                                    lockRoot.addScaledVector(lockNrm, sink);
            ...
            if (ap.hairStyle === "long" || maskGather) {
                ...
                const maneReach = (u) => {
                    ...
                    return Math.max(0.6, Math.min(2.4, (_mrA.y + skullY - coifHemY + MASK_SHOW) / MANE_DEEP));
                };
                let live = 0;
                for (let i = 0; i <= 48; i++)
                    live = Math.max(live, maneMass(mix(Math.PI - maneArc, Math.PI + maneArc, i / 48)));
                if (live >= 0.06)
            ...
            if (ap.hairStyle === "braids") {
                ...
                for (const s2 of [-1, 1]) {
                    ...
                    if (hooded || (style.mask && !coifed))
                        continue;
                    if (coifed && !masked && awayFromFace(rootU) > coifRim(0) - 0.10)
                        continue;
                    ...
                    const bp = (t, out) => {
                        ...
                        out.set(bRoot.x + (masked ? Math.sin(rootU) : s2) * (swingOut * swing), bRoot.y - drop * t, bRoot.z + (masked ? Math.cos(rootU) * swingOut * swing : swingFwd * swing));
        ...
        if (ap.beardStyle !== "none") {
            ...
            const hank = (u) => 0.30 * Math.cos(u * 7.3 + 0.4) + 0.15 * Math.cos(u * 12.9 - 1.1);
            const rag = (u) => 1 + 0.125 * Math.cos(u * 5.1 + 2.2) + 0.075 * Math.cos(u * 9.7 - 0.6);
        ...
        if (helmed) {
            ...
            const capY = (z) => {
                ...
                for (let i = 0; i < CAP.length - 1; i++) {
                    ...
                    if (z <= b[0])
                        return mix(a[1], b[1], (z - a[0]) / (b[0] - a[0]));
                }
            ...
            if (style.cheek === "short") {
                ...
                const deepHem = (u) => cheekHemAt(Math.abs(u));
            ...
            if (style.nape !== "none") {
                ...
                const fall = (u, v, inset, out) => {
                    ...
                    out.set(Math.sin(u) * (h.hw + c), y, zAt(v) + cu * (hd + c));
                };
            ...
            if (style.crown === "boar") {
                ...
                const bodyAt = (t, k) => {
                    for (let i = 0; i < BODY.length - 1; i++) {
                        ...
                        if (t <= b[0])
                            return mix(a[k], b[k], smooth(a[0], b[0], t));
                    }
                    return BODY[BODY.length - 1][k];
                };
                const boarSpine = (t, out) => {
                ...
                if (lod.trim) {
                    ...
                    for (const s of [-1, 1]) {
                        p.add(beast((t, out) => {
                            const a = t * 1.62;
                            out.set(s * (0.0066 + 0.0042 * t), tuskAt.y - 0.0080 + 0.0300 * (1 - Math.cos(a)), tuskAt.z + 0.0300 * Math.sin(a));
                        }, {
                            rows: 12, ring: 6,
                    ...
                    onBoar(0.80);
                    for (const s of [-1, 1]) {
                        p.add(rod(0.0010, 0.0055, 0.018, 5), brass, xf(_bq.x + s * 0.0075, _bq.y + 0.009, _bq.z - 0.004, -0.85, 0, -s * 0.55));
                    }
                    onBoar(0.845);
                    for (const s of [-1, 1]) {
            ...
            if (style.crown === "wyrm") {
                ...
                const wyrmSpine = (t, out) => {
                    ...
                    0.011 * Math.sin(t * Math.PI * 2.3) * (1 - clamp01((t - 0.68) / 0.32)), Math.max(capY(z), yTop - 0.030) + 0.012 + rise
                        // The head is thrown DOWN as well as forward, over the brow, so it
                        // looks at whoever the man is looking at.
                        - 0.030 * Math.pow(clamp01((t - 0.70) / 0.30), 1.5), z);
                };
                //          t     half     tall     scales
                const WYRM = [
                ...
                if (lod.trim) {
                    ...
                    for (const s of [-1, 1]) {
                        ...
                        p.add(rod(0.0010, 0.0044, 0.034, 5), brass, xf(h.x + s * 0.0100, h.y + 0.011, h.z - 0.006, -0.92, 0, -s * 0.42));
                    }
                    for (const s of [-1, 1]) {
            ...
            if (style.mask) {
                ...
                // made generous rather than the plates either side of it.
                {
                    ...
                    const vAt = (u, v, inset, out) => {
                        ...
                        out.set(Math.sin(u) * (a.hw - inset), y, a.z + c * (hd - inset));
            ...
            // the beard stay open, because a cowl that closes at the front is a bag.
            {
                ...
                const cowlFall = (t, s, inset, out) => {
                    ...
                    out.set(mix(Math.sin(u) * (R.x * tr + lift), Math.sin(u) * (R.x * 1.14 - inset), q), mix(skullY + R.y * Math.sin(a), mantleTopY - 0.010, s), mix(Math.cos(u) * (R.z * tr + lift), Math.cos(u) * (R.z * 1.08 - inset), q));
                };
                p.add(patch({
    ...
    if (ap.cloak !== "none") {
        ...
        const stackAt = (y) => stack * smooth(S.hemY - 0.12, S.hemY + 0.02, y + hipOff);
        ...
        emit("cloak", pivot, () => {
            ...
            if (_cloakSpy)
                _cloakSpy.rec = { origin: pivot.position.clone(), surf, lining: 0.014 };
...
export const BEARD_VALUES = (ARMOURY.find((s) => s.slot === "beard")?.options ?? []).map((o) => String(o.value));
/**
 ...
 */
export function beardSeatProbe(cls, seed, beardStyle) {
    ...
    c.group.traverse((o) => {
        ...
        if (!pos)
            return;
        if (name === `${RIG_TAG}torso`) {
            torsoMesh.push(mesh);
            return;
        }
        if (name === `${RIG_TAG}neck`) {
            neckMesh.push(mesh);
            return;
        }
        ...
        const n = idx ? idx.count : pos.count;
        for (let t = 0; t < n; t += 3) {
            beardTri.push(base + (idx ? idx.getX(t) : t), base + (idx ? idx.getX(t + 1) : t + 1), base + (idx ? idx.getX(t + 2) : t + 2));
        }
    });
    const nV = beardXYZ.length / 3;
    if (!nV)
        return { pieces: 0, throughMm: 0, worst: `${cls}/${beardStyle}`, overMm: 0, fallMm: 0, intoNeckMm: 0, neckWorst: `${cls}/${beardStyle}` };
    ...
    const join = (a, b) => { const ra = find(a); const rb = find(b); if (ra !== rb)
        parent[ra] = rb; };
    ...
    for (let t = 0; t < beardTri.length; t += 3)
        islands.add(find(rep[beardTri[t]]));
            ...
            for (let t = 0; t < nT; t += 3) {
                ...
                for (let ga = 0; ga <= G; ga++) {
                    for (let gb = 0; ga + gb <= G; gb++) {
                        ...
                        const wb = gb / G;
                        const wc = 1 - wa - wb;
                        v.set(t0.x * wa + t1.x * wb + t2.x * wc, t0.y * wa + t1.y * wb + t2.y * wc, t0.z * wa + t1.z * wb + t2.z * wc);
                        const yi = Math.round(v.y / YB);
                        let ring = rings.get(yi);
                        if (!ring) {
                            ring = new Float32Array(AZ);
                            rings.set(yi, ring);
                        }
    ...
    for (let i = 0; i < nV; i++) {
        ...
        if (Math.abs(Math.atan2(x, z)) > 0.9)
            continue;
