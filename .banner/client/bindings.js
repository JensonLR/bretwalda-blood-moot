 ...
 */
export const PLATFORM_MODIFIER_CODES = Object.freeze({
...
export function isPlatformModifier(code) {
 ...
 */
export function unsafeDefaultReason(code) {
 ...
 */
export function defaultsRuleViolations() {
    const out = [];
    const seen = new Map();
    for (const id of ACTION_IDS) {
        for (const code of DEFAULT_BINDINGS[id]) {
            ...
            if (why)
                out.push(`${id} defaults to ${code}: ${why}`);
            const already = seen.get(code);
            if (already)
                out.push(`${id} and ${already} both default to ${code}.`);
            else
                seen.set(code, id);
        }
        if (DEFAULT_BINDINGS[id].length === 0)
            out.push(`${id} ships with no binding at all.`);
    }
    return out;
}
// ---------------------------------------------------------------------------
...
function freeze(t) {
    const out = {};
    for (const id of ACTION_IDS)
        out[id] = Object.freeze([...t[id]]);
    return Object.freeze(out);
}
function isActionId(v) {
    return typeof v === "string" && ACTION_IDS.includes(v);
}
/**
...
function coerce(raw) {
    ...
    for (const id of ACTION_IDS) {
        ...
        if (!Array.isArray(v))
            continue;
        const codes = v.filter((c) => typeof c === "string" && c.length > 0)
 ...
 */
export function bindingsAreDefault(b = getBindings()) {
 ...
 */
export function bindingsTouchedHere() {
    return touchedHere;
}
export function subscribeBindings(onChange) {
    load();
 ...
 */
export function setBindingsPersister(fn) {
    persist = fn;
}
/**
 ...
 */
export function hydrateBindings(raw) {
    load();
 ...
 */
export function rebind(action, code, opts = {}) {
    load();
    if (!isActionId(action))
        return { ok: false, reason: "unknown", message: `No such action: ${action}`, conflicts: [] };
    if (!code)
        return { ok: false, reason: "unknown", message: "No key was pressed.", conflicts: [] };
    const reserved = reservedReason(code);
    if (reserved)
        return { ok: false, reason: "reserved", message: reserved, conflicts: [] };
    ...
    for (const id of clash)
        next[id] = next[id].filter((c) => c !== code);
 ...
 * is the same on both.
 */
const MAC_CODE_NAMES = Object.freeze({
...
export function labelForAction(action, join = " or ") {
 ...
 */
export function captureBinding(onCode) {
    let done = false;
    const stop = () => {
        if (done)
            return;
...
export function ensureKeyTracking() {
...
export function isActionDown(action, mouse) {
    for (const code of getBindings()[action]) {
        if (heldCodes.has(code))
            return true;
        if (mouse && code === "Mouse0" && mouse.left)
            return true;
        if (mouse && code === "Mouse2" && mouse.right)
            return true;
    }
    return false;
}
/** Down now, or tapped since the last sample. What one-shot deeds ask. */
export function isActionHit(action, mouse) {
    if (isActionDown(action, mouse))
        return true;
    for (const code of getBindings()[action])
        if (tappedCodes.has(code))
            return true;
    return false;
}
/** Called by `sampleInput` once the latch has been read. */
export function clearTapped() {
    tappedCodes.clear();
}
// ---------------------------------------------------------------------------
