 ...
 */
function groundBlade(rig: WarriorRig, piv: RigPivots, groundY: number): void {
  const weapon = rig.weapon;
  if (rig.gore.dropped.has(weapon)) return;
 ...
 */
function emoteLayer(kind: EmoteId, ph: number, shielded: boolean, seed = 0): void {
...
export function poseWarrior(
  ...
  if (motion.emote) {
    if (bodyOwned || staggered) {
      ...
      else emoteLayer(motion.emote, ph, carried, motion.seed);
    }
  }
