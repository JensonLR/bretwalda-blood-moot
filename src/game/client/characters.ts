  ...
  for (const o of rspy) {
    ...
    for (let i = 0; i <= NU; i++) {
      for (let j = 0; j <= NV; j++) {
        ...
        const cIn = skinClearance(skinHull, rIn);
        if (-cIn > punch) punch = -cIn;
        ...
        if (arc > 1e-4) {
          const ang = Math.atan(Math.abs(skinGap(formHull, rNext, rNrm) - fHere) / arc) * 180 / Math.PI;
