    ...
    for (const o of rspy) {
        ...
        for (let i = 0; i <= NU; i++) {
            ...
            for (let j = 0; j <= NV; j++) {
                ...
                if ((globalThis).__DBG === o.tag)
                    console.log(`DBG t=${t.toFixed(3)} s=${s.toFixed(3)} y=${(rIn.y * 1000).toFixed(1)} r=${(Math.hypot(rIn.x, rIn.z) * 1000).toFixed(1)} az=${(Math.atan2(rIn.x, rIn.z) * 180 / Math.PI).toFixed(0)} gap=${(gHere * 1000).toFixed(1)} fgap=${(fHere * 1000).toFixed(1)}`);
        ...
        if (helmed) {
            ...
            if (style.nape !== "none") {
                ...
                const hullAt = (y) => {
                    ...
                    let hw = sm(readP(sideP, y), S.neckHW);
                    let hd = sm(readP(backP, y), neckBackAt(y));
                    if (heavy) {
