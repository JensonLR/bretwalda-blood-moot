...
// ============================================================
// 4. BODY FITTINGS — the same question, below the neck
// ============================================================
//
...
//
// Sections 1-3 could not see a single one of these. They tap `headWear`, and a
...
const SINK_MM = 22;
console.log("");
console.log("[wear] 4. BODY FITTINGS — brooches, bosses, buckles, studs, rivets.");
...
// ============================================================
// 5. HANDS — which one is on which arm
// ============================================================
//
...
const PALM_MIN = 0.25;
console.log("");
console.log("[wear] 5. HANDS — chirality and palm facing, on the mesh as it renders.");
console.log("");
...
for (const cls of CLASSES) {
  for (const h of handProbe(cls, seeds[0])) {
    ...
    if (bad.length) hfails.push(`${cls} ${h.hand}: ${bad.join("; ")}`);
    console.log(
...
console.log(`[wear] ${hfails.length ? "FAIL" : "PASS"}: hands`);

process.exit(fails.length + gfails.length + bfails.length + hfails.length ? 1 : 0);
