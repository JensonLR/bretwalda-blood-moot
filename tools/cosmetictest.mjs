...
const mdRow = (r) => {
  ...
  return `| ${r.slot} | ${r.label} | ${r.cost}g | ${r.deltaE !== undefined ? r.deltaE.toFixed(1) : "—"} | ` +
    `${p ? pct2(p.sil) : "—"} | ${p ? pct2(p.form) : "—"} | ${f ? pct2(f.sil) : "—"} | ${f ? pct2(f.form) : "—"} | ` +
    `${p?.pix !== undefined ? pct2(p.pix) : "—"} | ${p?.verdict ?? (r.deltaE !== undefined ? (r.deltaE < JND ? "SAME COLOUR" : r.deltaE < LADDER_DE ? "NEAR" : "DIFFERS") : "—")} |`;
};
const md = [
  "# The cosmetics sweep",
  "",
  ...
  "",
  `**${OPTION_COUNT} options across ${ARMOURY.length} slots. ${wall.toFixed(0)} s wall clock, ${captures} rendered captures.**`,
  "",
  "## How to read it",
  "",
  ...
  `- **verdict** — IDENTICAL below ${IDENTICAL_PCT}% (the same object with two prices), FAINT below ${DIFFERS_PCT}%, else DIFFERS.`,
  "",
  ...
  "",
  "| helm | face covered | " + slotOf("hair").options.filter((o) => o.value !== "shaved").map((o) => o.label).join(" | ") + " |",
