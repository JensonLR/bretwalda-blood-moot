...
/**
 * The tangents of a control curve, cached on the curve itself.
 *
 * Fritsch–Carlson: the average of the two neighbouring secants, clipped so the
 * cubic below cannot overshoot a control point. The clip is what lets a table be
 * written as an outline rather than as a set of coefficients — no row can be
 * pushed past by its neighbours, so a nose 28 mm deep is 28 mm deep and not 33.
 */
const _tangents = new WeakMap();
const _tangentsFlat = new WeakMap();
function tangents(c, flat = false) {
    ...
    for (let i = 0; i < n - 1; i++)
        d[i] = (c[i + 1][1] - c[i][1]) / (c[i + 1][0] - c[i][0]);
    m[0] = d[0];
    m[n - 1] = d[n - 2];
    for (let i = 1; i < n - 1; i++)
...
/**
 * A curve through control points, crown-first — a monotone cubic Hermite.
 *
 * It was a smoothstep between adjacent rows, and that is a real defect rather
 * than a matter of taste: a smoothstep has ZERO DERIVATIVE AT BOTH ENDS, so a
 * table interpolated with one has a flat spot at every control row and a steep
 * run between them. On a curve used for a pin's residual that is invisible. On
 * a curve used for the geometry it is a set of horizontal terraces down the
 * forehead — plainly there in `art/shots/c1/head-turn.png`, one band per row of
 * `FRONT`, and it looks exactly like a contour map because it is one.
 *
 ...
 */
const resite = (c) => c.map(([y, v]) => [resiteY(y), v]);
/**
 ...
 */
const massEnvelope = (bearing, w) => 1 - smooth(w * 0.45, w, bearing);
/**
...
function pinTable(K) {
    ...
    for (let i = 0; i < PIN_N; i++) {
        ...
        const jit = J.pNose * bump(y - Y_TIP, 0, 0, 0.20, 1, 1) +
    ...
    for (let k = -rad; k <= rad; k++)
        w.push(Math.exp((-k * k) / (2 * sigma * sigma)));
    for (let i = 0; i < PIN_N; i++) {
        let acc = 0, wsum = 0;
        for (let k = -rad; k <= rad; k++) {
            const j = i + k;
...
const FACE = FACE_AUTHORED.map((l) => ({
 ...
 */
function across(p, a, b, q, hp, h, hq, t, endA, endB) {
    const dab = (b - a) / h;
 ...
 */
const rc = (c, y) => curve(c, y, true);
/** The loft between the edge loops, at one bearing and one row. */
function reliefAt(bear, y, out) {
    const n = FACE.length;
    if (bear >= FACE[n - 1].b)
        return out.set(0, 0, 0);
    let j = 0;
    while (j < n - 2 && bear >= FACE[j + 1].b)
        j++;
    const A = FACE[j];
    const B = FACE[j + 1];
    ...
    const endA = j === 0;
    const endB = j + 1 === n - 1;
    return out.set(across(rc(P.x, y), rc(A.x, y), rc(B.x, y), rc(Q.x, y), hp, h, hq, t, endA, endB), across(rc(P.y, y), rc(A.y, y), rc(B.y, y), rc(Q.y, y), hp, h, hq, t, endA, endB), across(rc(P.z, y), rc(A.z, y), rc(B.z, y), rc(Q.z, y), hp, h, hq, t, endA, endB));
}
/** A narrow weight in latitude, for handing one seed trait one landmark. */
const at1 = (t, w) => Math.exp((-t * t) / (w * w));
/**
 ...
 */
function faceSurfaceRaw(K, d, out) {
    const R = K.R;
    const F = K.F;
    const y = d.y;
    const sx = d.x < 0 ? -1 : 1;
 ...
 */
export function headMesh(cls, seed, nu = 40, nv = 44) {
    const S = skeleton(BUILD[cls]);
    const K = { R: S.headR, F: faceTraits(seed) };
...
export function headLandmarks(cls, seed) {
    const S = skeleton(BUILD[cls]);
    const K = { R: S.headR, F: faceTraits(seed) };
    const p = new THREE.Vector3();
    const d = new THREE.Vector3();
...
/**
 * A monotone warp that spends more of a fixed grid on one band of it: constant
 * density `d` inside `[a, b]` and 1 outside, integrated and inverted.
 *
 * NOT a change to the surface. The head is exactly the same shape before and
 * after; what moves is where its vertices are, and that is the difference
 * between a nose and a smudge. The measurement is arithmetic rather than a
 * matter of taste: at 40×44 the mesh's rows near the equator are 0.071 apart in
 * field `y`, and the whole nose — rhinion to subnasale — spans 0.14. TWO ROWS.
 * A nose cannot exist on two rows however it is authored, and this is, I think,
 * the real reason eight passes reached for ever-broader gaussians: a feature the
 * grid could not carry got smeared until it could, and a smeared feature is a
 * muzzle. Columns are the same story — the face is 32% of the bearing sweep and
 * was getting 32% of the columns while the back of the skull, which is a plain
 * arc, got the rest.
 *
 * The band gets 2.2× the density, which puts ~28 rows and ~20 columns on the
 * face for the same 3520 triangles, and the back of the head loses vertices it
 * had nothing to do with.
 */
function warpTable(lo, hi, a, b, d, soft) {
    const N = 512;
...
const faceRow = (s) => warpAt(ROW_CDF, s, -Math.PI / 2, Math.PI / 2);
/**
 ...
 */
const faceCol = (s) => warpAt(COL_CDF, s, 0, 1);
function headGeometry(K, nu, nv) {
    ...
    for (let j = 0; j <= nv; j++) {
        ...
        for (let i = 0; i <= nu; i++) {
            ...
            const u = Math.PI + faceCol(i / nu) * Math.PI * 2;
            faceSurface(K, dirOf(u, v, _d), p);
            pos.push(p.x, p.y, p.z);
            // The UV is the TRUE parameter, not the row index, so the warps above move
            // vertices without moving one texel of the complexion. `faceComplexion`
            // paints the brows, the lip tint and the stubble at fixed uv, and a grid
            // that renumbered its own rows would slide all three off the features.
            uv.push((u - Math.PI) / (Math.PI * 2), (v + Math.PI / 2) / Math.PI);
        }
    }
    const stride = nu + 1;
    for (let j = 0; j < nv; j++) {
...
const fissure = (f, tt) => f.hA * Math.pow(Math.max(0, 1 - tt * tt), 0.62);
/**
...
function addMouth(p, K, lod, place, M) {
    ...
    const half = (u) => 0.0155 * Math.pow(1 - Math.pow(clamp01(Math.abs(u) / w), 1.7), 0.55);
    p.add(headWear(K, {
        u0: -w, u1: w,
    ...
    emit("head", headPivot, () => {
        ...
        if (lod.trim) {
            ...
            for (const s of [-1, 1]) {
                ...
                p.add(ball(0.0062, 10), dark, xf(s * 0.0094 + K.F.asym, skullY + sub.y + 0.0018, sub.z - 0.0090, 0.52, s * 0.26, 0, 0.74, 0.42, 1.2));
            }
        }
        ...
        for (const s of [-1, 1]) {
            ...
            const arc = (u) => mix(inner, outer, clamp01((Math.abs(u) - 0.10) / 0.52));
        ...
        if (ap.hairStyle !== "shaved") {
            ...
            if (ap.hairStyle === "long") {
                ...
                for (const s of [-1, 1]) {
                    ...
                    ], Math.max(9, lod.limb + 1), { power: 2.0, wall: 0.013 }), hair, xf(s * R.x * 0.30, 0, 0, 0, 0, -s * 0.05));
                }
                if (lod.trim) {
                    ...
                    for (const s of [-1, 1]) {
                        for (let i = 0; i < 3; i++) {
                            ...
                            p.add(braid((t, out) => out.set(s * (R.x * (0.60 + off * 0.55)) * (1 - 0.20 * t), skullY + 0.03 - 0.34 * t, -R.z * (0.55 + 0.25 * Math.sin(t * 2.1)) - 0.02 * t), { strands: 2, turns: 0.35, rows: 7, ring: 5, radius: (t) => 0.016 * (1 - 0.55 * t * t) }), hair);
                        }
                    }
                }
            }
            if (ap.hairStyle === "braids") {
                ...
                for (const s of [-1, 1]) {
                    p.add(braid((t, out) => {
                        ...
                        out.set(s * (R.x * 0.94 + 0.052 * swing), skullY + R.y * 0.30 - 0.335 * t, -R.z * 0.20 + 0.055 * swing);
                    }, { turns: 3.4, rows: Math.max(12, lod.limb * 2), ring: Math.max(5, lod.limb - 2),
                        radius: (t) => 0.0165 * (1 - 0.45 * t * t) }), hair);
                    p.add(ring(0.0105, 0.0034, 4, 9), brass, xf(s * (R.x * 0.94 + 0.048), skullY + R.y * 0.30 - 0.318, -R.z * 0.20 + 0.050, Math.PI / 2 - 0.28, 0, s * 0.16));
                }
            }
        }
        // ---- beard ----
        ...
        if (ap.beardStyle !== "none") {
            ...
            if (!style.mask) {
                ...
                // lip as well, and without it the crop read as a chinstrap.
                {
                    ...
                    // the corner.
                    const swell = (u, v) => 0.0010 + 0.0052 * mo * Math.sin(Math.PI * clamp01(v))
                    ...
                    const leaf = (u) => mHalf * Math.pow(Math.sin(Math.PI * clamp01((Math.abs(u) - 0.025) / 0.365)), 0.5);
            ...
            if (ap.beardStyle === "full") {
                ...
                const belly = (k, lean) => p.add(shell(BELLY.map((b) => ({ y: skullY - b.drop, hw: b.hw * k, hd: b.hd * k, z: b.z })), Math.max(8, lod.limb), { power: 2.15, capTop: true, capBottom: true }), beard, xf(0, 0, 0, 0, 0, lean));
                /** The belly's own half-width, half-depth and offset at a given drop. */
                const bellyAt = (drop) => {
                    const last = BELLY[BELLY.length - 1];
                    if (drop <= BELLY[0].drop)
                        return BELLY[0];
                ...
                if (lod.trim) {
                    ...
                    for (let i = 0; i < 11; i++) {
                        ...
                        p.add(braid((t, out) => {
                            ...
                            out.set(Math.sin(a) * hw2, skullY - fall, B2.z + Math.cos(a) * hd2 + 0.070 * Math.pow(past, 1.2));
                        }, {
                            strands: 2,
                            turns: 0.9 + 0.5 * j,
                ...
                for (const s of [-1, 1]) {
                    ...
                    ], Math.max(8, lod.limb), { power: 2.1, capBottom: true }), beard, xf(s * 0.028, 0, 0.004, 0.10, 0, -s * 0.56));
                }
            }
                ...
                // radius to spare.
                const bPath = (t, out) => out.set(0, skullY - 0.186 - 0.272 * t, 0.036 + 0.100 * Math.pow(t, 1.15));
                const bRad = (t) => 0.0212 * (1 - 0.38 * t * t);
                p.add(shell([
        ...
        if (helmed) {
            ...
            if (lod.trim && style.bowl !== "cone") {
                ...
                for (let i = 0; i < 4; i++) {
                    ...
                    p.add(helmWear(K, {
                        ...
                        lift: (_u, s) => bowlLift(s) + (style.bowl === "shallow" ? 0.009 : 0.006)
            ...
            if (style.nape !== "none") {
                ...
                const sweep = (inset) => (t, v, out) => fall(mix(Math.PI + half, Math.PI - half, t), v, inset, out);
                p.add(patch({
                ...
                if (style.noble) {
                    ...
                    const lip = (inset) => (t, v, out) => fall(mix(Math.PI + half - 0.03, Math.PI - half + 0.03, t), mix(0.86, 1, v), inset, out);
